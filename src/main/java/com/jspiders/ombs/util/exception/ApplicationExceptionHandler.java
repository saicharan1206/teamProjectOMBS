...
public class ApplicationExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ErrorStructure> userEmailAlreadyFound(EmailAlreadyFoundException exp) {
		...
		error.setRootCause("Email is Already Exists!!");

		return new ResponseEntity<ErrorStructure>(error, HttpStatus.FOUND);
	}
	
	@ExceptionHandler
	public ResponseEntity<ErrorStructure> userRoleNotAccepted(UserRoleNotApplicableException exp){
		ErrorStructure error=new ErrorStructure();
		error.setStatusCode(HttpStatus.NOT_FOUND.value());
		error.setMessage(exp.getMessage());
		error.setRootCause("UserRole is Not Applicable!!");
		
		return new ResponseEntity<ErrorStructure>(error,HttpStatus.NOT_FOUND);
	}
	
	@ExceptionHandler
	public ResponseEntity<ErrorStructure> userEmailNotExists(UserNotFoundByEmail exp){
		ErrorStructure error=new ErrorStructure();
		error.setStatusCode(HttpStatus.NOT_FOUND.value());
		error.setMessage(exp.getMessage());
