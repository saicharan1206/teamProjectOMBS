...
import com.jspiders.ombs.util.exception.UserAlreadyExsistException;
...
public class UserServiceImpl implements UserService {
	...
	{
		user2.setUserFirstName(user.getUserFirstName().toLowerCase());
		user2.setUserLastName(user.getUserLastName().toLowerCase());
		user2.setUserRole(user.getUserRole().toLowerCase());
	...
	{
		throw new UserAlreadyExsistException("User is Already Exist");
	}
