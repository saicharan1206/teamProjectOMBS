...
public class UserServiceImpl implements UserService {
	...
	public ResponseEntity<ResponseStructure> userLogin(LoginRequestDTO loginRequestDTO) {
		User findByEmail = repo.findByEmail(loginRequestDTO.getEmail());
		
		if(findByEmail==null) {
			throw new EmailNotFoundException("Email Not Found in Database");
		}
		else if(!(findByEmail.getPassword().equals(loginRequestDTO.getPassword()))){
			throw new PasswordMismatchException("Password doesnot match");
		}
		
		UserResponseDTO userResponse = new UserResponseDTO();
		userResponse.setUserId(findByEmail.getUserId());
		userResponse.setUserFirstName(findByEmail.getUserFirstName());
		userResponse.setUserLastName(findByEmail.getUserLastName());
		userResponse.setEmail(findByEmail.getEmail());
	...
	@Override
	public ResponseEntity<String> sendMail(String to, String username, String role){
...
//		to = userRequestDTO.getEmail();
		...
		mailMessage.setText("change your password here by clicking on the below link");
		javaMailSender.send(mailMessage);
